<template>
  <div class="room-page">
    <el-card class="room-card hover-card" shadow="hover">
      <template #header>
        <div class="card-header">
          <div>
            <h3>自习室管理</h3>
            <p>维护自习室基础信息，并查看对应座位概况。</p>
          </div>
    ...
    <el-dialog v-model="batchDialogVisible" title="批量生成座位" width="520px" destroy-on-close>
      ...
      <template #footer>
        ...
        <el-button type="primary" @click="submitBatch" :loading="batchSaving">生成</el-button>
      </template>
    </el-dialog>

    <el-drawer v-model="seatDrawerVisible" title="座位概况" size="40%">
      <el-descriptions :column="1" border class="mb-4">
        <el-descriptions-item label="自习室">{{ currentRoom?.name }}</el-descriptions-item>
...
const submitBatch = async () => {
  ...
  try {
    await request.post(`/api/seats/rooms/${currentRoom.value.id}/seats/batch`, null, {
      params: batchForm
    })
